...
pathStub.youtube = function (configuration) {
  let searchCalled = 0;
  let videosCalled = 0;
  return {
    channels: {
      list(payload) {
        switch (payload.part) {
          ...
          default:
            return Promise.reject(new Error(`Missing mock response for: ${payload.part}`));
        }
      },
    },
    playlistItems: {
...
const YoutubeProxy = proxyquire('../../../lib/services/youtube', {
...
describe('Youtube Tests', function () {
  ...
  it('Gets a Channels Uploaded Playlist Id', function () {
    return yt.getChannelsUploadedPlaylistId(config.YOUTUBE_CHANNEL).then(function (response) {
  ...
  it('Gets a Channels Uploaded Playlist Id and caches it', function () {
    return yt.getChannelsUploadedPlaylistId(config.YOUTUBE_CHANNEL).then(function () {
  ...
  it('Gets live broadcast concurrent viewers count', function () {
    return yt.getChannelStatus().then(function (response) {
