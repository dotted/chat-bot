...
describe('SQLite Tests', function () {
  ...
  it('adds scheduled and gets commands', function (done) {
    const expected = [
      { sch_id: 1, sch_cmd_id: 1, cmd_id: 1, cmd_key: '!test', cmd_message: 'neat' },
    ];
    this.sql
      .addCommand('!test', 'neat')
  ...
  it('adds scheduled commands and gets them all', function (done) {
    const expected = [
      { sch_id: 1, sch_cmd_id: 1, cmd_id: 1, cmd_key: '!test', cmd_message: 'neat' },
      { sch_id: 2, sch_cmd_id: 2, cmd_id: 2, cmd_key: '!test2', cmd_message: 'cool' },
    ];
    this.sql
      .addCommand('!test', 'neat')
      .then(() => this.sql.addScheduledCommand('!test'))
  ...
  it('adds a banned phrase and gets it', function (done) {
    ...
    this.sql
      .addBannedPhrase('cool beans', 600, 'mute')
  ...
  it('gets all banned phrases', function (done) {
    const expected = [
      { text: 'cool beans', duration: 1000, type: 'ban' },
      { text: 'meepo meepo meep', duration: 1500, type: 'mute' },
    ];
    this.sql
      .addBannedPhrase('cool beans', 1000, 'ban')
  ...
  it('increments death', function () {
    const expected = [];
    return this.sql.incrementDeaths().then(() =>
  ...
  // skip this, as the timestamp generated for this is done within sqlite making it impossible to use fake timers
