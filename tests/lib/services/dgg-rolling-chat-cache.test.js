...
describe('Chat Cache Test suite', function () {
  describe('Chat Cache Viewer Map Tests', function () {
    it('add messages to the cache for a given user', function () {
    ...
    it('add messages to the cache for a given user up to the default of 2', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = {
    ...
    it('add messages to the cache for a given user and replaces messages if the number of messages is above the threshold', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = {
    ...
    it('add messages to the cache for multiple users', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = {
        linusred: ['hey nice meme man'],
        bob: ['really cool'],
    ...
    it('returns expected value for diffed messages that are the same', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = [1];
      chatCache.addMessageToCache('linusred', 'hey nice meme man');
    ...
    it('returns expected value for diffed messages that are diferent', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = [0.9];
    ...
    it('returns expected value for diffed messages that are crazy', function () {
      const chatCache = new RollingChatCache({ viewerMessageMinimumLength: 1 });
      const expected = [1];
 ...
 ( / )               ( \\\\ )
  ( /                 \\\\ )
   (_.·´¯\\\`·.¸¸.·´¯\\\`·.¸_)\``,
      ...
      const result = chatCache.diffNewMessageForUser(
        'linusred',
        `
      \`  (¯\\\`·.¸¸.·´¯\\\`·.¸¸.·´¯)
  ( \\\\                 / )
...
( ) (                 ) ( )
 ( / )               ( \\\\ )
  ...
  describe('Chat Cache Tombstoning Tests', function () {
    beforeEach(function () {
  ...
  describe('Chat Cache Rate Limit Tests', function () {
    beforeEach(function () {
      this.clock = sinon.useFakeTimers();
    });
  ...
  describe('Chat Cache Running List', function () {
    beforeEach(function () {
      this.clock = sinon.useFakeTimers();
      this.clock.tick(0);
    });
    ...
    it('adds many messages to running list', function () {
      const chatCache = new RollingChatCache({});
      const expected = [
        { user: 'linusred', message: 'hey nice meme man', timeStamp: 0 },
    ...
    it('replaces many messages when the queue is full', function () {
      ...
      const expected = [
        { user: 'linusred', message: 'eugh', timeStamp: 0 },
